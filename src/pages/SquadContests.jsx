...
import { ChevronLeft, Trophy, Flame, Star, Medal, Crown } from 'lucide-react';

export default function SquadContests() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      base44.entities.ContestEntry.list(),
      base44.entities.SquadMember.filter({ user_email: base44.auth.me().then(u => u.email).catch(() => '') })
    ]).then(async ([user, contests, entries, squads]) => {
      ...
      <div className="min-h-screen text-white pb-24 relative overflow-y-auto" style={{ backgroundColor: '#080810' }}>
        ...
        <div className="relative z-10 px-4 pt-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-2xl font-bold">🏆 Squad Contests</h1>
          </div>
          <div className="text-center py-20">
            <span className="text-5xl mb-4 block">🎉</span>
            <p className="font-bold text-lg text-white mb-2">No active contests</p>
            <p className="text-gray-400 text-sm">Check back soon for the next challenge!</p>
          </div>
        </div>
      ...
      <div className="relative z-10 px-4 pt-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400">Rise together, win together</p>
          </div>
        </div>
        ...
        {/* Leaderboard */}
        <div className="mb-6">
          <h2 className="text-sm font-bold tracking-widest text-gray-400 mb-3">LEADERBOARD</h2>
          <div className="space-y-3">
            {leaderboard.map((entry, idx) => {
              const rank = idx + 1;
              const isTop3 = rank <= 3;
              const isMySquad = userSquads.some(s => s.squad_id === entry.squad_id);
              
              return (
                <div
                  key={entry.squad_id}
                  onClick={() => navigate(`/glow-squads/${entry.squad_id}`)}
                  className={`rounded-2xl p-4 cursor-pointer transition hover:scale-[1.02] ${
                    isMySquad ? 'ring-2 ring-pink-500' : ''
                  }`}
                  style={{ 
                    background: isTop3 
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="font-semibold text-white text-sm">{entry.squad_name}</p>
                          {isMySquad && <p className="text-[10px] text-pink-400">Your Squad</p>}
                        </div>
                        <div className="text-right">
        ...
        {userSquads.length > 0 && (
          <div>
            <h2 className="text-sm font-bold tracking-widest text-gray-400 mb-3">YOUR SQUAD</h2>
            <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)' }}>
              <p className="text-xs text-gray-400 mb-2">Keep grinding! Every point counts 💜</p>
              <div className="flex items-center gap-2 text-sm">
                <Flame className="text-orange-400" size={16} />
                <span className="text-gray-300">Daily check-ins earn points for your squad</span>
              </div>
            </div>
          </div>
        )}
